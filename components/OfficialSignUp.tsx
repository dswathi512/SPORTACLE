...
import { User, UserRole, Sport, Gender, OfficialRole } from '../types';
    ...
    <div className="min-h-screen bg-brand-light flex flex-col justify-center items-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
            <UserGroupIcon className="w-16 h-16 text-brand-primary mx-auto" />
            <h2 className="mt-4 text-3xl font-extrabold text-brand-dark">Official & Coach Registration</h2>
            <p className="mt-2 text-brand-gray">Access tools to manage and discover talent.</p>
        </div>
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <InputField name="contact" label="Email ID" type="email" value={formData.contact} onChange={handleChange} />
            <InputField name="organization" label="Organization / Department" type="text" value={formData.organization} onChange={handleChange} />
            <SelectField name="officialRole" label="Your Role" value={formData.officialRole} options={OFFICIAL_ROLES_LIST} onChange={handleChange} />
            <SelectField name="gender" label="Gender" value={formData.gender} options={GENDERS_LIST} onChange={handleChange} />
            <SelectField name="sport" label="Primary Sport Focus" value={formData.sport} options={SPORTS_LIST} onChange={handleChange} />
